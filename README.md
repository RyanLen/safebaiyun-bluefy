...
这是一个 throwaway Spike，用来验证 SafeBaiyun 的 BLE 挑战—应答流程能否在 iOS Bluefy 中完成。页面不保存 MAC 或 `PRODUCT_KEY`，也不加载任何第三方 JavaScript。
...
1. 新建仓库，只上传 `index.html`，不要把 MAC 或 `PRODUCT_KEY` 写进文件。
2. 在仓库 Settings → Pages 中选择从分支部署。
3. 等 GitHub Pages 给出 `https://...github.io/.../` 地址。
4. 在 iPhone 的 Bluefy 中打开该 HTTPS 地址。
5. 页面三项诊断都通过后，临时输入 MAC 和 `PRODUCT_KEY`。
6. 靠近门锁，点击“选择设备并尝试解锁”，从 Bluefy 设备列表中选择门锁。
